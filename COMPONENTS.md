...
```svelte
<!-- template mode -->
<Grid mode="template" cols="minmax(0,0.82fr) minmax(0,1.18fr)" gap="2rem">
	<div>left column</div>
...
```svelte
<Drawer open={showDrawer} side="right" onclose={() => (showDrawer = false)}>drawer content</Drawer>
```
...
**Key props:** `open`, `anchor` (`HTMLElement`), `placement` (`bottom-start`, etc.), `offset`, `portal`, `class`
...
<span bind:this={anchorEl} style="display: inline-block;">
	<Button variant="ghost" size="sm" onclick={() => (showPop = !showPop)}>info</Button>
</span>
...
**Key props:** `items` (`{label, value, disabled?, destructive?}[]`), `trigger` (snippet), `open?`, `placement`, `offset`, `class`
...
```svelte
<DropdownMenu items={[{ label: 'edit', value: 'edit' }]} onselect={(v) => console.log(v)}>
	{#snippet trigger()}
...
**Key props:** `title` (optional), `sidebarWidth`, `class`
