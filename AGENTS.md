...
### Weight registers and themes

Weight registers are applied to `<body>` or a root container via `data-weight` or `applyWeight()`:
...
Themes are applied separately via `data-theme` or `applyTheme()`:

| theme     | character                     | use for                   |
| --------- | ----------------------------- | ------------------------- |
| `hextech` | brass, crystal, mechanical    | piltover-styled showcases |
| `arcane`  | shimmer, shards, organic glow | unstable themed showcases |

No weight or theme declared = default styles.
