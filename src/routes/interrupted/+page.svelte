<script lang="ts">
  ...
  function drawFrame(singleFrame = false) {
    ...
    for (let x = 0; x <= width; x += 4) {
      ...
      const noisy = time < noiseUntil
        ? (Math.random() - 0.5) * 54 * Math.sin(x * 0.19 + time * 0.22)
        : 0;
...
<div class="system-page">
  ...
  <div class="content-shell">
    ...
    <div class="cta-block" class:visible={showCta}>
      <div class="cta-row">
        <Button variant="secondary" onclick={() => window.location.reload()}>[ try the channel again ]</Button>
...
<style>
  .system-page {
    ...
    background:
      ...
      linear-gradient(180deg, color-mix(in srgb, var(--bg) 95%, var(--text) 5%) 0%, var(--bg) 34%, color-mix(in srgb, var(--bg) 82%, black) 100%);
  }
  ...
  :global(.fail-vignette) {
    background:
      ...
      radial-gradient(ellipse at center, transparent 56%, color-mix(in srgb, var(--status-fail) 10%, transparent));
  }
