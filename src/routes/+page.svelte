	...
	const examples = [
		{
			...
			description:
				'a dense monitoring dashboard. layered readouts, depth grid, live operator posture.',
			status: 'ok' as const,
			tag: 'dashboard'
		},
		{
			...
			description:
				'editorial work in the field-notebook register. warm, serif-forward, reads like a found document.',
			status: 'ok' as const,
			tag: 'editorial'
		},
		{
			...
			register: 'field-notebook',
			description:
				'a failure page that tells a story. terminal boot sequence, ambient noise, calm about being broken.',
...
<div class="nexus">
	...
	<div class="nexus-shell">
		<section class="nexus-hero" use:surface={{ delay: 0 }}>
			...
			>
				...
				{#snippet actions()}
					...
					<Button variant="ghost" href="https://github.com/edwardtufte/et-book"
						>[ font source ]</Button
					>
				{/snippet}
			</PageHeader>
			...
			<div class="nexus-intro">
				<Text variant="italic" color="muted">
					quiet confidence. technical depth. no ornamental sludge. every detail should look placed,
					not merely present.
				</Text>
			</div>
		...
		<Grid
			mode="template"
			cols="minmax(0, 1.7fr) minmax(17rem, 0.9fr)"
			gap="var(--space-xl)"
			class="nexus-main"
		>
			<section class="nexus-list-block" use:surface={{ delay: 120 }}>
				<div class="nexus-section-head">
					...
					<Text variant="body" color="muted"
						>three scenes. each one uses the system differently.</Text
					>
				</div>
