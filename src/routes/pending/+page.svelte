<script lang="ts">
  ...
  function drawFrame(singleFrame = false) {
    ...
    const secondaryX = centerX + Math.cos(secondaryProgress * Math.PI * 2 + Math.PI * 0.4) * width * 0.18;
    const secondaryY = centerY + Math.sin(secondaryProgress * Math.PI * 2 + Math.PI * 0.4) * height * 0.11;
  ...
  <div class="content-shell">
    ...
    <div class="cta-block" class:visible={showCta}>
      <div class="cta-row">
        ...
        <Button variant="ghost" class="cta-link" onclick={() => window.location.reload()}>[ check status ]</Button>
      </div>
